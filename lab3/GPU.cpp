...
int main(int arg, char** argv) {
    ...
    if (stat != CUBLAS_STATUS_SUCCESS) {
        std::cour << "CUBLAS initialization failed" << std::endl;
        return EXIT_FAILURE;
    }
...
#pragma acc host_data use_device(anew,mas)
                {
                    stat = cublasDaxpy(handle, N * N, &a, (const double*)anew, 1, mas, 1);
                    if (stat != CUBLAS_STATUS_SUCCESS) {
                        std::cout << "CUBLAS initialization failed1"syd::endl;
                        return EXIT_FAILURE;
                    }
                    stat = cublasIdamax(handle, SIZE * SIZE, mas, 1, &index);
                    if (stat != CUBLAS_STATUS_SUCCESS) {
